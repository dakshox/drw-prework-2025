...
# Check if inputs are strings, and convert them to datetime if necessary
    ...
    if isinstance(today_date, str):
        ...
        raise ValueError("today_date must be a date string or datetime object")
    
    # Check if past date is NaT
    if pd.isnull(past_date):
        raise ValueError("first_cpn_date is NaT")


    # Check if the past date is actually in the future
    if past_date > today:
        days_to_next_multiple = (past_date - today).days
        return first_cpn_date, float(days_to_next_multiple)

    # Start with the past_date and increment by 6 months until we find a date later than today
    next_six_month_multiple = past_date
    while next_six_month_multiple <= today:
        next_six_month_multiple += relativedelta(months=6)
    
    # Calculate the time intervals
    days_to_next_multiple = (next_six_month_multiple - today).days
    
    # Convert the interval to float (number of days)
    float_days_to_next_multiple = float(days_to_next_multiple)
    
    # Return the results
    return next_six_month_multiple.strftime("%Y-%m-%d"), float_days_to_next_multiple
...
def process_wrds_treasury_data(rawdata,keys_extra=[]):
    ...
    # map will turn each integer into the mapped string (or NaN if unmapped)
    data['ITYPE'] = data['ITYPE'].map(dict_type)
    # if you want to keep the original code for unmapped types:
    # data['ITYPE'] = data['ITYPE'].map(dict_type).fillna(data['ITYPE'])
...
def format_bond_df(df):
    if 'public size' in df.columns:     
        out = df.style.format({'ttm':'{:.2f}', 'ytm': '{:.2%}', 'price': '{:.2f}','ask': '{:.2f}','bid': '{:.2f}','cpn rate':'{:.2}','dirty price': '{:.2f}','accrual fraction': '{:.2f}','accrued int': '{:.2f}','quote date': "{:%Y-%m-%d}",'maturity date': "{:%Y-%m-%d}",'issue date': "{:%Y-%m-%d}",'public size': '{:.0e}'})
    else:
        out = df.style.format({'ttm':'{:.2f}', 'ytm': '{:.2%}', 'price': '{:.2f}','ask': '{:.2f}','bid': '{:.2f}','cpn rate':'{:.2}','dirty price': '{:.2f}','accrual fraction': '{:.2f}','accrued int': '{:.2f}','quote date': "{:%Y-%m-%d}",'maturity date': "{:%Y-%m-%d}",'issue date': "{:%Y-%m-%d}"})
